...
fn main() {
    ...
    let h = |t: f64| {
        ((t + 2.0 * PI / 3.0).sin().clamp(0.0, 1.0)
            - ((t + 2.0 * PI / 3.0) * 2.0).cos().clamp(0.0, 1.0))
            / -2.0
    };
    ...
    const R_SCALE: [f64; DIM] = [8.0, 4.5, 120.0];
    ...
    for frame in 0..frames {
        ...
        for i in 0..WIDTH {
            for j in 0..HEIGHT {
                if (progress as f64 + 1.0)
                    < (((j + i * HEIGHT) as f64) / ((WIDTH * HEIGHT) as f64)
                        * (PROGRESS_BAR_LEN as f64))
                {
                    progress += 1;
                    print!("█");
                    io::stdout().flush().unwrap();
        ...
        image.save(format!("video/frame{:0>8}.png", frame)).unwrap();
    }
}
