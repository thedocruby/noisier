...
pub struct NoiseMachine { randoms: [usize; usize::BITS as usize], } impl NoiseMachine {
    pub fn new(seed: u64) -> Self { 
        let mut rng = StdRng::seed_from_u64(seed); let mut randoms; rng.fill(&mut randoms); Self { randoms }
        ...
        let mut pos0 = Vec::with_capacity(dim); let mut spos = Vec::with_capacity(dim);
        for i in 0..dim { pos0[i] = pos[i].floor() as i64; spos[i] = pos[i] - pos0[i] as f64; }
        ...
        for i in 0..dim { let mut new_out = Vec::with_capacity(out.len()/2); 
        for j in 0..out.len()/2 { new_out[j] = int(spos[i], out[j*2], out[j*2 + 1]) }  out = new_out; } out[0]
